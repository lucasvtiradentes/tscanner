...
| File | Changes |
|------|---------|
| `packages/rust-core/crates/tscanner_config/src/config.rs` | Add `load_without_validation()`, make `validate_json_fields()` public, refactor `validate()` to return structured result |
| `packages/rust-core/crates/tscanner_cli/src/commands/config/command.rs` | Update `cmd_validate()` to run validation explicitly and show detailed output |
| `packages/rust-core/crates/tscanner_cli/src/config_loader.rs` | Maybe add variant that skips validation |
