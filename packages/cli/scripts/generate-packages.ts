...
async function main() {
  if (!process.env.CI && !process.env.GITHUB_ACTIONS) {
    console.log('This script should only run in CI/CD environment');
    process.exit(1);
  }
...
async function generateNativePackages(rootManifest: any) {
  console.log(`Step 1/2 - Generating native packages for ${PLATFORMS.length} platforms...`);
  ...
  for (const { platform, arch } of PLATFORMS) {
    ...
    if (existsSync(binaryPath)) {
      ...
      console.log(`   ✅ Generated package for ${platform}-${arch}`);
    } else {
      console.log(`   ⚠️  Binary not found for ${platform}-${arch} - ensure workflow copied it`);
    }
  }
}

async function updateCliPackageVersion(rootManifest: any) {
  console.log('Step 2/2 - Updating CLI package version...');
