...
impl FileWatcher {
            ...
            notify::recommended_watcher(move |res: Result<Event, notify::Error>| match res {
                Ok(event) => {
                    if let Err(e) = Self::handle_event(event, &tx) {
                        crate::utils::log_error(&format!("Error handling file event: {}", e));
                    }
                }
                Err(e) => crate::utils::log_error(&format!("Watch error: {:?}", e)),
            })?;
    ...
    fn handle_event(
        ...
        for path in event.paths {
            ...
            crate::utils::log_debug(&format!("File event: {:?}", file_event));
            tx.send(file_event)?;
        }
